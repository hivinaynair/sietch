...
The spent exhibit desk is `0xF94822401F3DdEC9e53c4143A4eFEdF61488dFA7`. A live walk needs a **fresh** T-bill and desk (same receipts, empty `usedTransfer`, 1 share on the desk, 0 on Paul’s books). From the repo root: `bun run rearm` deploys `ClipFactory` (or calls `rearm()` on it). Set `SIETCH_FACTORY_ADDRESS` + `SIETCH_CLERK_PRIVATE_KEY` on the web app. The page reads `factory.desk()`; **Re-arm** rotates the desk without a redeploy. The click submits `settle()` / `publishInbound`.
